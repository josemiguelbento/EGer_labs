...
[T1,T2,T3] = theoretical_bode_plots(10*10^3);
...
sheets = sheetnames(filename);
for i = 1:length(sheets)
    P2 = double(strrep(strrep(sheets(i),'R',''),'_','.'))*10^3;
    [k_theo,q_theo] = k_q_calc(P2);
    
    k_vec = [P2 k_theo 10^(data.(sheets(i)).K/20)];
    q_vec = [P2 q_theo 10^(data.(sheets(i)).Q/20)];
    
    K_pot = cat(1,K_pot,k_vec);
    Q_pot = cat(1,Q_pot,q_vec);
end
%% Functions
...
function [T1,T2,T3] = theoretical_bode_plots(P2)
    R1 =	5.10E+04;
    R2 =	1.00E+05;
    R3 =	1.00E+04;
    R4 =	1.00E+04;
...
function [K, Q] = k_q_calc(P2)
    ...
    K = 1/P2*sqrt((C2*R2*R4*R11)/(C1*R5));
    Q = R6*sqrt((C1*R5)/(C2*R2*R4*R11));
end